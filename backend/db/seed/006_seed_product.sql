-- The one real product. Stripe product/price created via the Stripe API directly (test
-- mode) — see the price id below. A$29 AUD, one-time.
--
-- Three product_contents rows, not one: this single purchase unlocks the template
-- file, the video lesson, AND Q001's full guidance body. Missing the question_set row
-- is the one mistake that would make the question page's paywall work in isolation and
-- then silently never unlock after a real purchase.
