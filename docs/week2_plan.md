...
| **4 — Storefront and packs** | **`[DONE]`** | `/store` shipped 2026-08-14: `Store.tsx`, `StoreSection`, `ContentTypeCard` (`components/store/`), routed in `App.tsx`, header nav consolidated (`MarketingLayout.tsx`), member rail updated (`MemberLayout.tsx`), axe sweep extended. `/courses` gained real price data (`CourseSummaryOut.product`) so the course card can show one. **The domain-pack SKU shipped 2026-08-15**, unblocked by the owner's direction to build the artefact from existing content: `scripts/build_domain_pack.py` typesets the real 100-question catalogue into a per-domain PDF, `db/seed/014` seeds it as an ordinary product (no new entitlement mechanism), `packs.py` + `PackDetail.tsx` (`/store/packs/:slug`) serve it, gating case 14 covers it. Risk (60 questions) is publish-ready; the other four domains are generated but too thin to publish honestly today (see ledger 32) |
...
| **6 — Admin hardening** | **`[PART]`** | Autosave (`useAutosave.ts` + `AutosaveIndicator.tsx`, wired into the lesson-body and block-text editors), inline blur validation (**shipped 2026-08-15** — `useFieldValidation.ts` + `FieldError.tsx`, every required field across all three content editors), `/admin/orders` (table + CSV export, `admin/orders.py`), and the manual-grant dialog + audit row (`ManualGrantDialog.tsx`, `POST /admin/entitlements/grant`) are all shipped. **Not done, and not code:** the non-developer usability test (§31.3), the vendor-risk IP confirmation, the 99 preview edits, and the §62 device QA sweep are human/owner actions this session cannot perform — see §7's DoD and the ledger for what's left of each |

**Where the week actually stands:** Phases 1–5 are built and fully accepted, non-negotiables included — the domain-pack SKU shipped 2026-08-15 once the owner unblocked it, rather than staying deferred. Phase 6's engineering is now complete: autosave, inline blur validation, `/admin/orders` and the manual grant are all shipped. What's left is entirely human or owner-only: the usability test, the IP confirmation, the real-device QA pass and the Week 2 report itself. Phase 0's mobile walkthrough and Week 1 sign-off closed 2026-08-14/15 — see [`week1_go_no_go.md`](week1_go_no_go.md).
...
- [~] At least one domain pack is purchasable end to end with a real artefact behind it — **or** it is formally deferred with the owner's agreement and the reason recorded. — *mechanism built and code-complete 2026-08-15 (see ledger 32–34); the Risk pack (60 questions, 31-page PDF) is content-ready to publish. What remains is not code: upload the PDF to Supabase Storage, create its Stripe Price, and re-run `db/seed/014_seed_domain_pack.py` with both — see `docs/pricing.md`-style note left in the seed script's own docstring*
...
- [~] Admin autosave works; `/admin/orders` reconciles; a non-developer has added a lesson unaided while watched. — *autosave, inline blur validation and `/admin/orders` all shipped; the usability test itself remains `[BLOCKED]` on a real non-developer, watched, unaided*
- [~] The Week 1 mobile walkthrough is done and the Week 1 go/no-go is closed. — *walkthrough done 2026-08-14 (two defects found, both fixed); the report is written as [`week1_go_no_go.md`](week1_go_no_go.md) with an explicit **GO, conditionally**. Closed on my side; `week1_plan.md:394` — the owner's response — is the remaining half*
...
| **24** | **Which domains ship as packs first** — all five, or one as a proof? Risk (60 questions) is the only one past the thin-catalogue floor today; the other four are generated but flagged (see ledger 32). | Phase 4 seeding |
...
- ~~#14 Verify a Resend sending domain.~~ **Answered 2026-08-15: no — leave it as it is.** The owner does not currently have a domain to verify. This is a decision, not a non-answer, and it is recorded here as one: the consequence named throughout this document stands deliberately rather than by default — `email_service.py` stays on Resend's sandbox sender, so **no real customer can be emailed** until this is revisited. Nothing here blocks Week 3 engineering; it blocks the first real sale, per [`week1_go_no_go.md`](week1_go_no_go.md) §5.
- ~~#15 Account ownership~~ / ~~#16 GitHub repo access~~ **Answered 2026-08-15: every account and the repo belong to the owner.** The largest non-technical risk on the project since Week 1 Day 1 is closed. No handover-continuity gap remains on this point.
- ~~#19 What a domain pack contains.~~ **Answered 2026-08-15: design it from the files already in the repo, rather than wait on new author copy.** Built the same day — see ledger 32. The pack is generated from `docs/questions/questions.json` (the same real content already live on the site), typeset and put in a working order derived from the tags already on each question (tier → regulator pressure → effort), not hand-authored. This is a genuine answer to the decision, not a workaround of it: the brief's no-placeholder rule is satisfied because every word in the PDF is the author's own, already published and already reviewed.
...
1. ~~The domain-pack PDF product~~ (W2-R6) — **shipped 2026-08-15**, no longer a cut candidate; see ledger 32–34.
...
- [x] Autosave works and is visible; validation is inline on blur. — *Autosave: on the two rich-text drafts (lesson body, block text/callout) — every 20s plus on blur, "Saved HH:MM" visible, never clears the draft on failure. **Narrower than the full spec**: not extended to every admin field (titles, slugs, video/file pickers) — those are single-action selects or short fields with negligible loss-on-refresh risk, not the "40 minutes of typed guidance" case §31.8 names. Inline-blur validation: **done 2026-08-15** — every required field across all three content editors now validates on blur rather than only at submit; see ledger 43*
...
**Status column marked against the repository on 2026-08-15.** 43 of 49 done · 2 `[UNVERIFIABLE]` · 1 `[PART]` · 3 `[TODO]`, and **all three remaining are blocked on a human, not on engineering time.** Non-negotiables #9 and #11 (tasks 12, 15), Phase 4's storefront and the domain pack (29–34), Phase 5's legal + analytics (35–41), all of Phase 6's engineering (autosave, blur validation, orders, manual grant — 42, 43, 44, 45), and Phase 0's mobile walkthrough and Week 1 go/no-go (4, 5) have all closed. Decisions #14, #15, #16 and #19 were answered by the owner 2026-08-15 (§8.4) — #14 as a deliberate "leave it as is," #15/#16 as "every ownership belongs to me," #19 by unblocking the pack to be built from existing content.

**No open item is engineering.** The usability test (46), the IP confirmation (47), and the §62 device QA sweep (48) all need a person this session cannot substitute for. The Week 2 report (49) is done — [`week2_report.md`](week2_report.md). See §0.5.
...
| 4 | Mobile walkthrough at 375px | 0 | W2-R1 | — | **`[DONE]`** — walked by the owner on a real device 2026-08-14; two defects found and fixed (hero search truncation at 375px, scroll position carried across every route change). See [`week1_go_no_go.md`](week1_go_no_go.md) §3 |
| 5 | Week 1 go/no-go | 0 | W2-R1 | — | **`[DONE]`** — [`week1_go_no_go.md`](week1_go_no_go.md), 2026-08-14. **GO, conditionally.** The owner's response (`week1_plan.md:394`) is the one half that cannot be written here |
...
| 32 | Pack product type + page | 4 | W2-R6 | — | **`[DONE]`** — 2026-08-15, unblocked by the owner's direction to design the artefact from existing content rather than wait on new copy. `scripts/build_domain_pack.py` typesets the real 100-question catalogue into a per-domain PDF (31 pages for Risk); `db/seed/014_seed_domain_pack.py` seeds it as an ordinary product (one `template` row + N `question_set` rows — **no new entitlement mechanism**, per RS 5.6); `app/api/v1/content/packs.py` + `PackDetail.tsx` (`/store/packs/:slug`) serve it. Risk (60 questions) is publishable; Cyber/Compliance/Resilience/AI are generated but flagged thin (`MIN_QUESTIONS_TO_PUBLISH = 20`) — see §7's DoD note |
| 33 | Pack in My Library | 4 | W2-R6 | — | **`[DONE]`** — no new code: `/me/library` already lists any owned `template` row under Templates and any `question_set` grant under Reference, and a pack is built from exactly those two content types. Known rough edge, not a defect: a 60-question pack purchase lists 60 individual Reference rows rather than one collapsed pack entry — acceptable for Week 2, worth a grouped view later if a second pack ships |
| 34 | Pack gating case | 4 | W2-R2 | — | **`[DONE]`** — case 14, four tests in `tests/gating/test_gating.py`: the PDF 401s anonymous, 403s a signed-in non-owner, and 200s once purchased (same `has_access_to_or_admin` path as any other template — no new gate to bypass); a pack's `question_set` grant is proven **not** to change question access at all. `content_graph`'s `pack_pdf`/`pack_question`/`pack_product` fixtures added in `conftest.py`. All 53 backend tests pass |
...
| 43 | Inline validation on blur | 6 | W2-R9 | — | **`[DONE]`** — 2026-08-15. `lib/useFieldValidation.ts` (dependency-free — installed but unused `react-hook-form`/`zod` deliberately left alone rather than introduced for the first time across three editors in one pass) + `components/ui/FieldError.tsx`, wired into every required field across all three content editors: `AdminQuestions` (title, domain, guidance), `AdminTemplates` (title, description), `AdminCourses` (course title/description, module title, lesson title, both video-attach modals' Mux ids, the file-block template picker). Validates on blur; `validateAll` re-checks at submit so a pasted-and-submitted field is still caught. Values and validity are fully decoupled — the hook never touches a field's value — which is what makes "never cleared because another failed" true by construction rather than by care |
...
| 49 | Week 2 report + go/no-go | 6 | — | — | **`[DONE]`** — [`week2_report.md`](week2_report.md), 2026-08-15. **GO for Week 3** — every engineering item closed; what remains (usability test, IP confirmation, device QA) is human-only |

## 29. Risk watchlist

| Risk | Watch for | Mitigation this week |
|---|---|---|
| **`[ACCEPTED, 2026-08-15]` No real customer can be emailed at all.** `email_service.py`'s own docstring: every send is redirected to the owner's inbox on Resend's sandbox sender. **This is now a deliberate holding position, not an open item** — decision #14 was answered "leave it as it is," since no domain is available to verify yet | Any receipt or sale-notification email arriving `[Not delivered to buyer]`-prefixed at `OWNER_NOTIFICATION_EMAIL` instead of the buyer | None scheduled. Revisit before the first real sale is treated as evidence of anything — see [`week1_go_no_go.md`](week1_go_no_go.md) §5 |
...
| **A domain pack that sells something already free.** The fastest possible way to lose buyer trust | Product copy implying purchase unlocks the questions | §20.6's honesty notice, above the previews, not in fine print. **`[CLOSED 2026-08-15]`** — `packs.py`'s `HONESTY_NOTICE` is returned by the API itself (not frontend copy that could drift), rendered above the price on `PackDetail.tsx`, and repeated verbatim on the PDF cover by `build_domain_pack.py` — gating case 14 also proves purchasing the pack does not change question access at all |
...
| ~~Decision #15 (account ownership) still unanswered~~ | — | **`[CLOSED 2026-08-15]`** — every account and the repo belong to the owner. Was the largest non-technical risk on the project since Week 1 Day 1 |
