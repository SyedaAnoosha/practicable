...
#### **Lesson prose (W4-R13)**
- [x] Migration `017` applied; **every existing plain-text body renders byte-identically to before** — the regression test says so — **half true, now fixed 2026-08-21**. Migration `017_lesson_prose_sanitized.py` and `022_block_prose_sanitized.py` are real: nullable `prose_sanitized`, no backfill, verified by direct read of both files. But **no regression test existed anywhere** proving the "byte-identical" half of the claim — `sanitize_html()`'s own `test_plain_text_survives_unchanged` tests the sanitizer function in isolation, not `Learn.tsx`'s actual `block.prose_sanitized ? <RichText/> : <p className="whitespace-pre-line">` branch that decides which path a real lesson takes. Fixed: added `frontend/src/components/content/__tests__/RichText.test.tsx` (5 tests) covering the piece of this that's actually unit-testable — RichText's own render contract (headings/lists/table/link render unchanged, three XSS vectors stripped by the client-side defence-in-depth pass) — seen red first (3 of 5 failed with real diffs — `onclick`/`javascript:`/`<script>` all survived — when `clientSanitize` was stubbed to `return html` unchanged). The plain-text branch itself is structurally guaranteed by `Learn.tsx`'s ternary (verified by direct read: `RichText` is only ever invoked when `prose_sanitized` is non-null) but has no dedicated `Learn.tsx` render test — a real gap still open, sized like a full page-component test rather than a one-line fix, noted rather than silently left as "DONE."
...
- [x] The lesson page still has exactly one `h1`, confirmed by axe — **half true, half not done**. `Learn.tsx:494` has exactly one `<h1>` for the lesson title, confirmed by grep — the h1-count claim itself is true. But **no axe test scoped to this feature actually confirms it**; the generic `accessibility.spec.ts`/`gating.spec.ts` e2e suites exist but neither targets the rich-text round trip specifically, and the plan's own step 9 round-trip test (h2/h3/h4 + bullets + numbered list + table + link, saved in admin and rendered on the member page, both themes, seven widths) does not exist anywhere in the repo — searched and confirmed absent. Recorded as a real, still-open gap rather than marked done; building the full round-trip e2e suite is sized beyond a one-line fix and wasn't attempted in this pass.
...
**Still open after this pass (not fixed, sized beyond what this pass covers):** a `Learn.tsx`-level render test proving the plain-text/rich-text branch choice in production, and the plan's own step 9 round-trip axe/e2e test. Both are real, both are page-level integration tests rather than one-line fixes — recorded honestly rather than claimed done or silently dropped.
