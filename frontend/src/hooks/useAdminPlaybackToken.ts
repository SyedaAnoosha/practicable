...
// One of the four states the admin preview must tell apart — "still encoding" and
// "Mux lost the asset" are as real as a working player, and a player that fails
// silently on either reads as broken software, not a video that isn't ready yet.
...
/**
 * Fetch a signed Mux playback token for admin video preview, plus the asset's live
 * encoding state — `Media.status` in the database is set optimistically at attach
 * time and isn't kept in sync with Mux afterward, so only a live check can answer
 * "is this actually ready to play."
 *
